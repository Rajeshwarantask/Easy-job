...
export interface ValidationRecord {
  input: { messageId: string; threadId: string; subject: string; sender: string; date?: string; platform: string; documentType: string; bodyText: string; structuredText: string; links: string[]; blockCount: number; htmlPresent: boolean };
  output: { success: boolean; role?: string; company?: string; location?: string; status?: string; eventDate?: string; confidence?: number; provenance?: unknown; error?: string };
...
export async function validateRawGmailCorpus(messages: RawGmailMessage[], mappingContext: MappingContext): Promise<ValidationRecord[]> {
  ...
  for (const message of messages) {
    ...
    records.push({
      input: { messageId: message.id, threadId: message.threadId, subject: decoded.headers.subject, sender: decoded.headers.from, date: message.internalDate, platform: platform.platform || "unknown", documentType: document.type || "unknown", bodyText, structuredText: cleaned.structuredText, links: links.map((link) => link.url), blockCount: cleaned.blocks.length, htmlPresent: Boolean(decoded.body.html) },
      output: { success: result.success, role: app?.role, company: app?.company, location: app?.location, status: app?.eventType, eventDate: date instanceof Date ? date.toISOString() : undefined, confidence: app?.parserConfidence, provenance: (app as any)?.candidateEvidence, error: result.error },
