...
const ROLE_WORDS = /\b(?:developer|engineer|designer|analyst|manager|intern|scientist|specialist|lead|architect|consultant|administrator|associate|director|sde|swe|qa|devops|tester|recruiter|coordinator|executive|apprentice|technician|consultant)\b/i;
...
const GENERIC_COMPANY_NOISE = /^(?:the hiring team|hiring team|recruiting team|talent acquisition|human resources|careers?|company|employer|organization|corporation|browser|with)$/i;
const COMPANY_SUFFIX = /\b(?:inc\.?|llc|ltd\.?|limited|corp\.?|corporation|plc|technologies|solutions|systems|labs|group|health|services|private limited)\b/i;
const LOCATION_WORDS = /\b(?:bengaluru|bangalore|chennai|hyderabad|mumbai|delhi|pune|kolkata|gurugram|gurgaon|noida|ahmedabad|jaipur|kochi|singapore|london|new york|san francisco|california|texas|remote|hybrid|on[- ]?site|onsite|work from home)\b/i;
const BAD_CONTEXT = /\b(?:professional network|building project|notification emails?|your message|delivery notice|browser|view job|apply with resume|unsubscribe|privacy policy|thank you for applying)\b/i;
const ROLE_NOISE = /^(?:recruiter|recruiting|hiring|talent|human resources|the team|company|job alert|notification|outreach|opportunities?)$/i;
const PERSON_ROLE_PREFIX = /^(?:[A-Z][a-z]+\s+[A-Z](?:\s+\([^)]*)?\s*\()/;
...
function add(list: CandidateEvidence[], field: CandidateField, value: string | undefined, source: CandidateSource, pattern: string, evidence: string, positiveScore: number, semanticType?: SemanticEntity) {
  ...
  if (!cleaned) return;
  const negativeScore = (/(?:message was sent|delivery notice|your message)/i.test(evidence) && pattern === "sent-to-company" ? 1.2 : 0) + (PLATFORM_NAMES.test(cleaned) ? 0.95 : 0) + (CTA.test(cleaned) ? 0.95 : 0) + (FOOTER.test(cleaned) ? 0.95 : 0) + (SENTENCE.test(cleaned) ? 0.75 : 0) + (HTML_NOISE.test(cleaned) ? 1.2 : 0) + (BAD_CONTEXT.test(cleaned) ? 1.0 : 0) + (TECHNOLOGY_LIST.test(cleaned) && field !== "role" ? 1.2 : 0) + (SIGNATURE.test(cleaned) ? 1.0 : 0) + (PERSON_LIKE.test(cleaned) && !ROLE_WORDS.test(cleaned) && field !== "person" && field !== "company" && field !== "location" ? 0.9 : 0) + (GENERIC_COMPANY_NOISE.test(cleaned) && field === "company" ? 1.0 : 0) + (field === "role" && /,/.test(cleaned) ? 0.8 : 0) + (field === "role" && !ROLE_WORDS.test(cleaned) && cleaned.split(/\s+/).length > 5 ? 0.45 : 0);
  const valid = !HTML_NOISE.test(cleaned) && !SIGNATURE.test(cleaned) && !BAD_CONTEXT.test(cleaned) && !(field === "role" && (ROLE_NOISE.test(cleaned) || PERSON_ROLE_PREFIX.test(cleaned))) && (field === "company" ? isValidCompanyCandidate(cleaned) : field === "role" ? isValidRoleCandidate(cleaned) : field === "location" ? cleaned.length <= 70 && !FOOTER.test(cleaned) : cleaned.length <= 70 && !FOOTER.test(cleaned));
...
export function extractCandidates(subject: string, body: string, sender?: string): CandidateEvidence[] {
  ...
  const subjectSentTo = subject.match(/\b(?:was|were)\s+sent\s+to\s+(.+?)(?:\s*[-–—|].*|$)/i)?.[1] || subject.match(/\b(?:application|applied|submitted)\s+(?:to|at)\s+(.+?)(?:\s*[-–—|].*|$)/i)?.[1];
  if (subjectSentTo) add(candidates, "company", subjectSentTo, "subject", "subject-company-relation", subject, 0.98);
  const subjectAtPair = subject.match(/^(.+?)\s+at\s+(.+)$/i);
  if (subjectAtPair && ROLE_WORDS.test(subjectAtPair[1])) {
    add(candidates, "role", subjectAtPair[1], "subject", "subject-role-at", subject, 0.98);
    add(candidates, "company", subjectAtPair[2], "subject", "subject-company-at", subject, 0.96);
  }
  const subjectRole = subject.match(/^(?:application\s+(?:for|to)|your application\s+(?:for|to)|position|role|job)\s*[:\-]?\s*(.+)$/i)?.[1]
    || subject.match(/\b(?:for|as)\s+(?:the\s+)?(.+?)(?:\s+(?:at|with)\s+|$)/i)?.[1];
  if (subjectRole && ROLE_WORDS.test(subjectRole)) add(candidates, "role", subjectRole, "subject", "subject-role", subject, 0.96);
  ...
  const suffixCompany = /\b([A-Z][A-Za-z0-9&.' -]{2,90}\b(?:inc\.?|llc|ltd\.?|limited|corp\.?|corporation|plc|technologies|solutions|systems|labs|group|health|services|private limited))\b/gi;
  for (const match of text.matchAll(suffixCompany)) add(candidates, "company", match[1], "pattern", "company-suffix", match[0], 0.86);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (ROLE_WORDS.test(line) && line.length <= 100) {
      const rolePrefix = line.match(/^(.{0,90}?\b(?:developer|engineer|designer|analyst|manager|intern|scientist|specialist|lead|architect|consultant|administrator|associate|director|sde|swe|qa|devops|tester|recruiter|coordinator|executive|apprentice|technician)\b)/i)?.[1] || line;
      add(candidates, "role", rolePrefix, i === 0 ? "subject" : "heading", "role-shaped-line", line, i === 0 ? 0.84 : 0.78);
    }
    ...
    const location = line.match(LOCATION_WORDS);
    if (location && line.length <= 80) add(candidates, "location", location[0], i === 0 ? "subject" : "paragraph", "location-keyword", line, i === 0 ? 0.86 : 0.76);
  }
...
export function selectCandidates(candidates: CandidateEvidence[]) {
  ...
  for (const field of ["company", "role", "location", "person"] as CandidateField[]) {
    ...
    const subjectWinner = ranked.find((candidate) => candidate.source === "subject" && candidate.confidence >= 0.9);
    const winner = subjectWinner || ranked[0];
...
export function resolveCandidates(subject: string, body: string, sender?: string) {
  ...
  for (const field of ["company", "role", "location"] as const) {
    if (!selected[field]) {
      const direct = candidates
        .filter((candidate) => candidate.field === field && !candidate.rejected && (candidate.confidence >= 0.9 || (field === "role" && candidate.source === "heading" && candidate.confidence >= 0.75)))
        .sort((a, b) => b.confidence - a.confidence)[0];
      if (direct) selected[field] = direct;
    }
  }
