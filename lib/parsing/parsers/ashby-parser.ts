/**
 * Ashby Platform Parser (Enhanced)
 * 
 ...
 * - Date/time parsing with timezone
 * - Salary extraction
...
export class AshbyParser implements PlatformParser {
  ...
  parse(
    ...
    } else if (/interview.*scheduled|interview.*invitation|interview|next.*stage|interview confirmed/i.test(fullText)) {
      eventType = "interview";
      eventConfidence = 0.85;
    } else if (/offer|congratulation|we.{0,10}excited|move.*forward|advance|job offer|extended.*offer/i.test(fullText)) {
      eventType = "offer";
      eventConfidence = 0.85;
    } else if (/unfortunately|regret|not.*moving|not.*selected|rejected|not moving forward/i.test(fullText)) {
      eventType = "rejection";
      eventConfidence = 0.85;
    } else if (/assessment|screening|test|evaluation|coding|take.*test/i.test(fullText)) {
    ...
    const companyPatterns = [
      /(?:at|for\s+[A-Za-z\s]+\s+at)\s+([A-Z][A-Za-z&\s]{3,40})(?:\n|,|!|$)/,
    ...
    const rolePatterns = [
      /(?:for|application\s+for|position)\s+(?:the\s+)?(?:"|')?([^"',\n]{5,50})/i,
      /Applied for\s+([^,\n]+)/i,
    ];
    ...
    // Extract from Ashby URLs
    if (!requisitionId) {
      const urlJobMatch = body.match(/\/job\/([A-Za-z0-9-]+)/i);
      if (urlJobMatch?.[1]) {
    ...
    const interviewerMatch = body.match(/(?:with|interviewer|speak\s+with|meet\s+with|from)\s+([A-Za-z\s]+?)(?:\n|,|\(|$)/);
