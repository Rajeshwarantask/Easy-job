...
const coinbase = resolveCandidates("Application submitted to Coinbase, Inc", "Rajeshwaran, was sent to Coinbase, Inc");
assert.equal(coinbase.selected.company?.value, "Coinbase, Inc");
assert.equal(coinbase.selected.role, undefined);
...
const semanticFixture = resolveCandidates("Application update", "Senior Platform Engineer\nNorthwind Systems Ltd\nRemote");
assert.equal(semanticFixture.candidates.some((candidate) => candidate.source === "semantic-nlp" && candidate.semanticType === "JOB_TITLE"), true);
assert.equal(semanticFixture.candidates.some((candidate) => candidate.source === "semantic-nlp" && candidate.semanticType === "ORGANIZATION"), true);
assert.equal(resolveCandidates("Hello Rajeshwaran", "Rajeshwaran, Soul Developer\nReact, TypeScript, HTML, CSS").selected.role, undefined);
