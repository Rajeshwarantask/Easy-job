...
type SyncRange = { from?: string | null; to?: string | null };
type GmailMessage = { id: string; threadId: string; payload?: Record<string, unknown>; snippet?: string; internalDate?: string };

async function gmailFetch(path: string, accessToken: string) {
  const response = await fetch(`${GMAIL_API}${path}`, { headers: { Authorization: `Bearer ${accessToken}` }, cache: "no-store" });
  if (!response.ok) {
...
function getGmailQuery(range: SyncRange) {
  const query = [BASE_RECRUITMENT_QUERY];
  if (range.from) query.push(`after:${Math.floor(new Date(`${range.from}T00:00:00.000Z`).getTime() / 1000)}`);
  if (range.to) {
    const nextDay = new Date(`${range.to}T00:00:00.000Z`);
    nextDay.setUTCDate(nextDay.getUTCDate() + 1);
    query.push(`before:${Math.floor(nextDay.getTime() / 1000)}`);
  }
  return query.join(" ");
}
...
function toDashboardApplication(application: NonNullable<ParseResult["application"]>) {
  ...
  const status = eventType.includes("reject") ? "rejected" : eventType.includes("offer") ? "offer" : eventType.includes("interview") ? "interview" : eventType.includes("assessment") || eventType.includes("test") ? "assessment" : "applied";
  ...
  return { ...application, id: original?.gmailMessageId || application.applicationId || crypto.randomUUID(), company: application.company || "", role: application.role || null, location: application.location || null, status, platform: application.parsedBy || "gmail", appliedDate: date, lastUpdated: date, interviewDate: interviewEvent?.date instanceof Date ? interviewEvent.date.toISOString() : null, interviewTime: interviewEvent?.time || null, interviewLink: interviewEvent?.details?.interviewLink || null, timezone: interviewEvent?.timezone || null, jobUrl: application.jobUrl || null, assessmentLink: null, recruiterName: null, recruiterEmail: null, salary: null, gmailThreadId: original?.gmailThreadId || null, parserVersion: application.parserVersion || "1.0.0", confidence: application.parserConfidence || 0, originalEmail: original, parserApplication: application };
}

async function fetchAllMessageIds(accessToken: string, range: SyncRange) {
  const ids: Array<{ id: string; threadId: string }> = [];
  let pageToken: string | undefined;
  do {
    const params = new URLSearchParams({ q: getGmailQuery(range), maxResults: "500" });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ processed: result.processed, applications: successful.map((entry) => toDashboardApplication(entry.application!)), errors: [...diagnostics.errors, ...result.errors.map((entry) => `${entry.gmailMessageId ?? "unknown"}: ${entry.error}`)], diagnostics, syncDurationMs: Date.now() - startTime, syncedAt: new Date().toISOString() });
  } catch (error) {
